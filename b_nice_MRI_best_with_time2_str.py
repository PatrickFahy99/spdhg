...
for prox_niter in [1]:
    ...
    for choice in ['best']:
        ...
        C = odl.uniform_discr([-1, -1], [1, 1], [d1, d2],
                              dtype='complex')  # Complex space
        ...
        for i in range(n):
            ci = C.element(mps[i])
            # Ci(x) = x.*ci (element-wise)
            Ci = ops.RealMultiplyOperator(X, ci)
        ...
        for i in range(n):
            bi = R(C.element(ksp[i]))                   # Data points
                ...
                0.5*odl.solvers.functional.L2NormSquared(A[i].range).translated(bi))

        f = odl.solvers.SeparableSum(*fi)               # Data fidelity

        g = fctls.resetTV(X, reg_term=l1)                # TV Regularizer
        ...
        if model == 'L2':
            ...
            if titles_on:
                plt.title('SIGPY Reconstruction, $\lambda=${}'.format(l))
            plt.show()
        else:
            sol = X.element(sol)
            #sol = R(C.element(sol))
            plt.imshow(M(sol), cmap='gray', vmax=contrast), plt.axis('off')
            if titles_on:
                plt.title('PDHG Reconstruction, $\lambda=${}'.format(l))
            plt.show()
        ...
        def callback(step, n, f, g, A, sol):
            cb = (odl.solvers.CallbackPrintIteration(fmt='iter:{:4d}', step=step, end=', ') &
                  odl.solvers.CallbackPrintTiming(
                      fmt='time: {:5.2f} s', cumulative=True, step=step)
        ...
        def spdhg_batch0(sampling, b, gamma, model, l,  n, f, g, A, nepoch, nruns, prox_niter):
            ...
            for i in range(nruns):
                ...
                # Ob.Fun. f(Ax) +g(x)
                temp[0, i] = cb.callbacks[1].out
                temp[1, i] = cb.callbacks[1].err              # error x^k - x*
                # distance x^k - x^k-1
                temp[2, i] = cb.callbacks[1].dis
                # computational time
                temp[3, i] = cb.callbacks[1].time
            ...
            if titles_on:
                plt.title(
                    sampling+' sampling, $b={}$, $\lambda=${}'.format(b, l))
            plt.show()
        ...
        lb = len(b_list)
        if os.path.isfile('new_results_'+model+'_time2_str/'+filename+'_'+sampling+'_'+str(prox_niter)+'.npy') and not redo_spdhg:

            Results = np.load('new_results_'+model+'_time2_str/' +
                              filename+'_'+sampling+'_'+str(prox_niter)+'.npy')
            s = Results[0*lb:1*lb]
            e = Results[1*lb:2*lb]
            d = Results[2*lb:3*lb]
            ...
            for j, b in enumerate(b_list):
                ...
                if j != 0 and j != lb-1:
                    results = np.load('results_theta/' +
                                      filename+'_b{}.npy'.format(b))

                    amin = int(results[nbatches+1, 0])
                    if len(results[0]) > 1:
                        amax = int(results[nbatches+1, 1])
                    amin_u = int(results[nbatches+2, 0])
                    if len(results[0]) > 1:
                        amax_u = int(results[nbatches+2, 1])
                ...
                i = j
                s[i], e[i], d[i], s_var[i], e_var[i], d_var[i], time[i] = spdhg_batch(
                    sampling, b, 10**gamma[i])
                print('---------------')
            ...
            if redo_spdhg and overwrite_spdhg_recon:
                ...
                np.save('new_results_'+model+'_time2_str/'+filename +
                        '_'+sampling+'_'+str(prox_niter)+'.npy', results)
        ...
        if titles_on:
            plt.title(
                '$||x^k-x^*||/||x^*||$ for MRI Reconstruction \n $n=${}, $\lambda=${}'.format(n, l))
        plt.xlabel('epochs', fontsize=14)
        # plt.xlim(0,nepoch)
        # plt.ylim(10**-5,1)
        # plt.xlim(0,400)
        ...
        for j, i in enumerate(I):
            plt.plot(time[i], e[i], color=colors[j],
                     linewidth=lw, label=lgnd[i])
        plt.legend(fontsize=18, frameon=True, facecolor='white')
        if titles_on:
            plt.title(
                'Error $||x^k-x^*||/||x^*||$ over time \n for MRI with $n=${}, $\lambda=${}'.format(n, l))
        ...
        plt.legend(fontsize=18, frameon=True, facecolor='white')
        if titles_on:
            plt.title(r'$||x^{k}-x^{k-1}||\frac{1}{||x^{k-1}||}$' +
        ...
        if titles_on:
            plt.title(
                '$\Phi_r(x^k)$ for MRI Reconstruction \n $n=${}, $\lambda=${}'.format(n, l))
        plt.xlabel('epochs', fontsize=14)
        plt.yscale('log')
        # plt.xscale('log')
        # plt.xlim(0,nepoch)
