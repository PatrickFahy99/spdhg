...
for auxiliar in [1]:
    ...
    for choice in ['best']:
        ...
        lb = len(b_list)
        if os.path.isfile('new_results_'+model+'_time2_str_epoch/'+filename+'_'+sampling+'_'+str(auxiliar)+'.npy') and not redo_spdhg:

            Results = np.load('new_results_'+model+'_time2_str_epoch/'+filename+'_'+sampling+'_'+str(auxiliar)+'.npy')
            s = Results[0*lb:1*lb]
            e = Results[1*lb:2*lb]
            d = Results[2*lb:3*lb]
            ...
            if redo_spdhg and overwrite_spdhg_recon:
                ...
                np.save('new_results_'+model+'_time2_str_epoch/'+filename+'_'+sampling+'_'+str(auxiliar)+'.npy',results)
