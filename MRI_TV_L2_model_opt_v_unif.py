...
for Prox_Niter in ['balanced']:
    ...
    for choice in ['best']:
        ...
        #if os.path.isfile('results_'+model+'/'+filename+'_'+sampling+'_'+Prox_Niter+str(prox_niter)+'.npz') and not redo_spdhg:
        if 0:
            print('Loading existing results...')
            results = np.load('results_'+model+'/'+filename+'_'+'bserial'+'_'+Prox_Niter+str(prox_niter)+'.npz')
            Results = results['r']
            s = Results[0*lb:1*lb]
            e = Results[1*lb:2*lb]
            d = Results[2*lb:3*lb]
            ...
            results = np.load('results_'+model+'/'+filename+'_'+'bfair'+'_'+Prox_Niter+str(prox_niter)+'.npz')
            Results = results['r']
            s2 = Results[0*lb:1*lb]
            e2 = Results[1*lb:2*lb]
            d2 = Results[2*lb:3*lb]
            s_var2 = Results[3*lb:4*lb]
            e_var2 = Results[4*lb:5*lb]
            d_var2 = Results[5*lb:6*lb]
            time2  = Results[6*lb:7*lb]
            theta2 = results['t']
            ...
            if redo_spdhg and overwrite_spdhg_recon:
                ...
                np.savez('results_'+model+'/'+filename+'_'+sampling+'_'+Prox_Niter+str(prox_niter)+'.npz', r=results, t=theta)
        ...
        for i,b in enumerate(b_list):
            lgnd.append(r'$b={}$'.format(b)+' opt')
        
        lgnd2= []
        for i,b in enumerate(b_list):
            lgnd2.append(r'$b={}$'.format(b)+' unif')
        ...
        for j,i in enumerate(J):
            if i!=n: plt.plot( e[i]**2, color=colors[j], linewidth=lw, label=lgnd[i])# (5, (10, 3)) )
            plt.plot( e2[i]**2, color=colors[j], linewidth=lw, label=lgnd2[i], linestyle= '-.' )
            if show_rate and l>0:
                b = b_list[i]
                aux = np.linspace(0,nepoch,nepoch+1)
                rate = 10**-2*theta[i]**(aux*n//b)
                plt.plot( rate, color=colors[j], linewidth=lw, linestyle='dashed')
        plt.legend(fontsize=18, frameon=True, facecolor='white', ncol=2)
        plt.hlines(10**-4, 0, 40, linestyles= '--', color='gray')
