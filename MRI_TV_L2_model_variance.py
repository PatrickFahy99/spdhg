...
for Prox_Niter in ['balanced']:
    ...
    for sampling in samplings:    
        ...
        lb = len(b_list)
        if os.path.isfile('results_'+model+'_variance/'+filename+'_'+sampling+'_'+Prox_Niter+str(prox_niter)+'.npz') and not redo_spdhg:

            results = np.load('results_'+model+'_variance/'+filename+'_'+sampling+'_'+Prox_Niter+str(prox_niter)+'.npz')
            Results = results['r']
            s = Results[0*lb:1*lb]
            e = Results[1*lb:2*lb]
            d = Results[2*lb:3*lb]
            ...
            if redo_spdhg and overwrite_spdhg_recon:
                ...
                np.savez('results_'+model+'_variance/'+filename+'_'+sampling+'_'+Prox_Niter+str(prox_niter)+'.npz', r=results, t=theta)
        ...
        #%%
        lgnd[0],lgnd[1] = r'$b=1$'+'\t(SPDHG)',r'$b=12$'+'\t (PDHG)'
       ...
       #%%
        ...
        if nruns>1:
            ##### Plot Distance to Solution ||x^k - x*|| (max & min)
            aux = np.linspace(0,nepoch,nepoch+1)
            for j,i in enumerate(J):
                plt.plot( e[i], color=colors[j], linewidth=lw, label=lgnd[i] )
                plt.fill_between( aux, s_var[i], d_var[i], color=colors[j], alpha=al )
            plt.legend(fontsize=22, frameon=True, facecolor='white')
            #if titles_on: plt.title('$||x^k-x^*||/||x^*||$ for MRI Reconstruction \n $n=${}, $\lambda=${}'.format(n,l))
            plt.xlabel('epochs',fontsize=18)
            plt.ylabel(r'$e_b^2$'+'\t\t',fontsize=18,rotation=0)
        ...
        if nruns>1:
            ##### Plot Distance to Solution ||x^k - x*|| (log max & min)
            aux = np.linspace(0,nepoch,nepoch+1)
            for j,i in enumerate(J):
                plt.plot( e[i], color=colors[j], linewidth=lw+0.7, label=lgnd[i] )
                plt.fill_between( aux, s_var[i], d_var[i], color=colors[j], alpha=al )
                b = b_list[i]
                #aux = np.linspace(0,nepoch,nepoch+1)
                rate = 2*10**-2*theta[i]**(aux*n//b)
                plt.plot( rate, color=colors[j], linewidth=lw+0.7, linestyle='dashed')
