...
nruns = 1                                       # Repeats
rho = 0.98                                          
prox_niter = 12                                 # FISTA Iterations per epoch

fixed_prox_niter = True                        
if fixed_prox_niter:
    prox_niter = 5                              # FISTA Iterations per iteration

b_list = [1,2,3,4,6,12]                         # b = 1,...,n
#b_list=[1,12]
lb = len(b_list)
...
# Sampling
#for sampling in ['bserial','bnice']:
for sampling in ['bserial']:
    ...
    for choice in ['best']:
        ...
        lb = len(b_list)
        if os.path.isfile('new_results_'+model+'_time3/'+filename+'_'+sampling+'_'+choice+'_.npy') and not redo_spdhg:

            Results = np.load('new_results_'+model+'_time3/'+filename+'_'+sampling+'_'+choice+'_.npy')
            s = Results[0*lb:1*lb]
            e = Results[1*lb:2*lb]
            d = Results[2*lb:3*lb]
            ...
            if sampling=='bserial':                            # Optimal gammas obtained numerically
                gamma = [-0.6,-0.9,-1.1,-1.2,-1.3,-1.6]
            else:
                gamma = [-0.6,-0.9,-1.0,-1.2,-1.3,-1.6]
            ...
            for j,b in enumerate(b_list):
                ...
                if fixed_prox_niter:
                    ...
                    prox_niter_b = prox_niter//nbatches
                print('prox_niter =',prox_niter_b)
                
                if j!=0 and j!=lb-1 and sampling=='bserial':
            ...
            #### Save Results
            if overwrite_spdhg_recon:
                results = np.concatenate([s, e, d, s_var, e_var, d_var, time])
                np.save('new_results_'+model+'_time3/'+filename+'_'+sampling+'_'+choice+'_.npy',results)
